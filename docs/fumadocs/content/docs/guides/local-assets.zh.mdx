...
镜像始终是显式 opt-in。Studio 派发的子进程只继承你 shell 中导出的 `HF_ENDPOINT`，默认不会被重定向到任何第三方镜像。若只想让 WorldFoundry 管理的子进程走镜像，导出 `WORLDFOUNDRY_HF_ENDPOINT=https://hf-mirror.com`；对这些子进程它优先于继承的 `HF_ENDPOINT`。
