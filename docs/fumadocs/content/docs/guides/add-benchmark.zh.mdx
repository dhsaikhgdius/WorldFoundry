...
WorldFoundry 当前公开支持的 benchmark surface 是 Benchmark Zoo。新增 benchmark 支持要走同一条路径：catalog manifest、external task manifest、runner contract 与 readiness evidence。
...
Benchmark runner 把 framework integration 与 leaderboard eligibility 分开。Official-validation import 通过，只能算有用的开发证据；leaderboard 声明仍然需要 official scorer 或 judge、声明清楚的资产，以及完整的 scorecard evidence。
...
Reviewer 应该能把 catalog 里的声明一路追溯到具体命令与产物：
