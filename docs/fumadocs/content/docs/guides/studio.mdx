...
| `spark` | 3DGS | Starts the standalone Spark 3DGS viewer. Vendor JS under `worldfoundry/studio/assets/vendor/` is gitignored — run `python scripts/studio/fetch_vendor_assets.py` once after clone. |
...
each run's own manifest and scorecard. Only curated nontrivial examples are
embedded here.
