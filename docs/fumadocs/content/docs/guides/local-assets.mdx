...
Mirrors are always opt-in. Studio-dispatched child processes inherit whatever `HF_ENDPOINT` your shell exports and are never redirected to a third-party mirror by default. To route only WorldFoundry-managed child processes through a mirror, export `WORLDFOUNDRY_HF_ENDPOINT=https://hf-mirror.com`; it takes precedence over an inherited `HF_ENDPOINT` for those children.
