...
第一份模型 PR 的范围要窄：一个模型族、一个 operator、一个 pipeline、必要时一个 runtime profile、一条已解析的 runner path，以及一份聚焦的 run-evidence report。
...
WorldFoundry 通过同一套公开 contract 解析每个 runnable method。在加任何捷径之前，先确认这几条规则：
...
光有 manifest claims 不算数。Support matrix 会从 staged evidence 重新计算 readiness。
...
推理时必须使用的 transforms 可以留在对应 runtime package 中，例如 GroundingDINO `util.transforms`、DUST3R cropping/transforms、DROID-SLAM RGB-D geometry helpers。Training datasets、dataloaders、losses、callbacks、augmentation-only modules 与 experiment scripts 不应进入 runtime package，除非这个框架将来开始支持该模型族的仓内训练。
