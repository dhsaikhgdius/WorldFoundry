...
## Optional native kernels (ABI-qualified)

`packages/worldfoundry-native-kernels` is an **opt-in** wheel, deliberately kept
out of the main `worldfoundry` package and out of `[all]`. It only ships a C++
dispatcher DSO smoke path today; it does not change default model execution.

- Build and install only inside the exact PyTorch/CUDA env you intend to
  qualify (`python -m build --wheel --no-isolation packages/worldfoundry-native-kernels`).
- Import is side-effect free; call `inspect()` / `load()` explicitly at a
  prewarm boundary so ABI mismatches fail closed.
- CPU-only hosts should skip this package. Details:
  `packages/worldfoundry-native-kernels/README.md` and
