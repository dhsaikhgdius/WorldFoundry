...
| `WORLDFOUNDRY_HF_DOWNLOAD_LOCK_TIMEOUT` | no | Seconds to wait for the cross-process HF download `FileLock` (default `7200`; `-1` waits forever). |
| `WORLDFOUNDRY_HF_DOWNLOAD_LOCK_WAIT_LOG_SECONDS` | no | How often waiting processes log while blocked on that lock (default `30`). |
...
Multi-rank jobs that call `maybe_download_hf_repo_on_rank0` broadcast download
success/failure on the default process group. Large Hub fetches can exceed the
default NCCL collective timeout — prefer a long-timeout Gloo backend (or raise
the process-group / `NCCL_TIMEOUT`) for that coordination, and keep the HF cache
on a filesystem every participating rank can read after the broadcast.
