...
| `WORLDFOUNDRY_HF_DOWNLOAD_COORDINATOR` | no | HF multi-rank download coordinator: `local` (default, each node's local-rank0) or `global` (world rank0 only; **requires a shared HF cache filesystem**). |
...
`maybe_download_hf_repo_on_rank0` defaults to per-node local-rank0 downloads so
node-local HF caches still populate under torchrun. Set
`WORLDFOUNDRY_HF_DOWNLOAD_COORDINATOR=global` only when every rank can read the
same HF cache path after rank0 finishes (shared filesystem).
