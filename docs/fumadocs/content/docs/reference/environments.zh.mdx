...
| `WORLDFOUNDRY_HF_DOWNLOAD_COORDINATOR` | 否 | 多 rank HF 下载协调：`local`（默认，每节点 local-rank0）或 `global`（仅 world rank0；**要求共享 HF cache 文件系统**）。 |
...
`maybe_download_hf_repo_on_rank0` 默认按节点 local-rank0 下载，以便 torchrun
下节点本地 HF cache 仍能物化。仅当所有 rank 在 rank0 完成后都能读到同一
HF cache 路径（共享文件系统）时，才设置
`WORLDFOUNDRY_HF_DOWNLOAD_COORDINATOR=global`。
