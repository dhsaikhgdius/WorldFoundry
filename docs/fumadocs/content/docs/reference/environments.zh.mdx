...
| `WORLDFOUNDRY_HF_DOWNLOAD_LOCK_TIMEOUT` | 否 | 跨进程 HF 下载 `FileLock` 等待秒数（默认 `7200`；`-1` 表示一直等）。 |
| `WORLDFOUNDRY_HF_DOWNLOAD_LOCK_WAIT_LOG_SECONDS` | 否 | 等待该锁时的日志间隔秒数（默认 `30`）。 |
...
多 rank 任务若走 `maybe_download_hf_repo_on_rank0`，会在默认进程组上
broadcast 下载成败。大体积 Hub 拉取常会超过默认 NCCL collective 超时——下载
协调请优先用长超时的 Gloo（或提高进程组 / `NCCL_TIMEOUT`），并保证参与
broadcast 的各 rank 在结束后都能读到同一份 HF cache（共享文件系统前提）。
