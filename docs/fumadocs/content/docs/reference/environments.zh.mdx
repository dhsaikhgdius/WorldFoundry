...
结构化日志（`configure_logging`、JSONL 文件 sink、字段序列化）会脱敏敏感
**键名**（分段匹配，如 `api_key` / `hf_token`）以及已知 **值前缀**（OpenAI/HF
的 `sk-`/`hf-`、GitHub `ghp_`/`github_pat_`、Slack `xox*`、AWS `AKIA…`、以及以
`eyJ…` 开头的紧凑 JWT）。库外可复用 `redact_sensitive_text`。

**原始 stdout/stderr 捕获刻意保真**，**不会**经过上述 scrubber。任务/控制台
tee 可能另有 argv/行级脱敏；不要假设磁盘上每一字节都已脱敏。

文件滚动参数（`configure_logging` 的 `rotation` / `retention`）：

| 后端 | `rotation` | `retention` |
| --- | --- | --- |
| loguru（已安装时默认） | 尺寸字符串，如 `10 MB` | 时间或数量字符串（loguru 语义） |
| stdlib 回退 | 解析为 `RotatingFileHandler.maxBytes` | 只解析为 **备份文件个数**（`1 week` → `7`）；不是按 mtime 过期 |

两边都按尺寸做 rotation；retention 措辞在统一 mtime 策略落地前按后端解释。
