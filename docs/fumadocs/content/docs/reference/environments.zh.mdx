...
## 可选 native kernels（ABI 限定）

`packages/worldfoundry-native-kernels` 是**可选** wheel，刻意不并入主
`worldfoundry` 包，也不进入 `[all]`。当前只提供 C++ dispatcher DSO 冒烟路径，
不会改变默认模型执行。

- 仅在要鉴定的那个精确 PyTorch/CUDA 环境内构建安装
  （`python -m build --wheel --no-isolation packages/worldfoundry-native-kernels`）。
- import 无副作用；在 prewarm 边界显式调用 `inspect()` / `load()`，ABI 不匹配时失败关闭。
- 仅 CPU 的主机请跳过该包。细节见
  `packages/worldfoundry-native-kernels/README.md` 与
