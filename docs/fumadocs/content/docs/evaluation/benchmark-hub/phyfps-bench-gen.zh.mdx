...
- 存在 mock 预测后端（`WORLDFOUNDRY_PHYFPS_PREDICT_BACKEND=mock`）；用它产出的 scorecard 会被标记为 mock runtime，不是官方证据。默认后端为 `official`。
- Meta FPS 对齐指标需要调用方提供 Meta FPS 值；完整 leaderboard 对齐需要完整官方 prompt 套件、生成产物与 Visual Chronometer checkpoint。
