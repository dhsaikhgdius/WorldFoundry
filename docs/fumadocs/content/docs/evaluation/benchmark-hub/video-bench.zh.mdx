...
scorecard 主键：`videobench_average`。
...
- 官方协议使用 hosted GPT-4o 评审打分，需要 `WORLDFOUNDRY_VIDEOBENCH_GPT4O_API_KEY` 或 `OPENAI_API_KEY`；没有凭据时不会进行任何评审。
- 官方视频数据集（`Video-Bench/Video-Bench_videos`）在 Hugging Face 上是 gated 的，需要单独申请访问。
