...
scorecard 主键：`videoscore_average`。
...
- catalog 中记录过针对 `video_feedback` 的有界 VideoScore scorer GPU 验证，但那不是 leaderboard 提交。
- 全套件 SPCC/成对 leaderboard 协议仍需完整的上游生成帧目录树与 VideoScore scorer 模型权重。
