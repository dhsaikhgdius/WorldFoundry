...
- catalog 标志为 `leaderboard_valid: false`、`official_benchmark_verified: false`（release status 为 `experimental`）；有界官方 GPU 验证只覆盖一个 dynamic 样例，完整套件验证仍为 pending（catalog blockers）。
- leaderboard 对齐需要完整 3,000 例套件的 `evaluation.json` 树以及官方 metric checkpoint 栈（DROID、GroundingDINO、SAM、RAFT/SEA-RAFT）；导入的结果只是对调用方数字的归一化拷贝。
- scorecard 主键：`worldscore_average`。
