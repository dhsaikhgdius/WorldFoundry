...
export WORLDFOUNDRY_T2VWORLDBENCH_PROMPT_FILE="$PWD/worldfoundry/data/benchmarks/assets/t2vworldbench/data/meta_data/meta_data.json"
```

`WORLDFOUNDRY_T2VWORLDBENCH_PROMPT_FILE` is the only prompt-related environment variable the runner reads; the vendored official runtime lives at `worldfoundry/evaluation/tasks/execution/runners/t2vworldbench/runtime/t2vworldbench` and the bundled prompt CSV at `worldfoundry/data/benchmarks/assets/t2vworldbench/prompt/prompt.csv`.
...
- The bundled `meta_data.json` is only a tiny in-tree example; full validation requires generated videos plus the manually staged official Drive metadata and sample assets.
- Leaderboard parity requires the upstream VLM runtime (a LLaVA-compatible checkpoint) and normalized official result files.
