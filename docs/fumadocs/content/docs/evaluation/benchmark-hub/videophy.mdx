...
- The VideoCon-Physics official runtime is integrated but has not been executed in this checkout; running it requires the `videophysics/videocon_physics` checkpoint and a CUDA GPU.
- Full validation requires generated videos for the official prompt suite; leaderboard parity requires full upstream scorer execution and normalized official result files.
