...
Primary scorecard key: `videobench_average`.

## Leaderboard notes

The official Video-Bench README publishes a cross-model leaderboard scored by a GPT-4o / GPT-4o-mini judge on 1–5 or 1–3 scales. Representative upstream baselines:
...
Video-Bench also reports stronger human alignment than existing automatic metrics. In its human-alignment table, Video-Bench reaches a Spearman correlation of **0.733** on imaging quality and **0.750** on color consistency, while individual VBench component metrics land around **0.363–0.545**.

A WorldFoundry scorecard is local evidence only. Reproducing the table above still requires the official prompt suite, GPT judge credentials, and a per-dimension video layout.
...
- The official protocol scores with a hosted GPT-4o judge and requires `WORLDFOUNDRY_VIDEOBENCH_GPT4O_API_KEY` or `OPENAI_API_KEY`; no judging happens without credentials.
- The official videos dataset (`Video-Bench/Video-Bench_videos`) is gated on Hugging Face; access must be requested separately.
