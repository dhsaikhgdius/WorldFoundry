...
- `WORLDFOUNDRY_MIRABENCH_SCORER_BACKEND=mock` writes deterministic placeholder metric values for wiring runs only; never report mock numbers as benchmark results.
- Full validation requires generated videos for the official 150-prompt suite plus the staged scorer checkpoints; leaderboard parity additionally requires official judge/checkpoint assets and normalized upstream result files.
