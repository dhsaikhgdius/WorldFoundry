...
- 本仓库只以有界的单视频官方运行执行过 `generative_numeracy` 组件；这不构成全套件证据。
- 四个基于 MLLM 的类别仍需兼容的 LLaVA runtime 与 checkpoint；`motion_binding` 依赖 DOT，检测类路径还需要各自类别的 Depth-Anything/SAM 资产。
- 全套件验证需要全部 1,400 条生成视频（每类 200 条）；完整官方基准与 leaderboard 对齐均未执行过。
