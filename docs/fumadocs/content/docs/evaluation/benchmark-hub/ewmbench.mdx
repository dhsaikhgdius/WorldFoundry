...
- The bundled task manifest is deliberately small. Use the Hugging Face dataset for full coverage.
- The default direct scorer backend is not the leaderboard scorer. It is a deterministic placeholder score for checking local command wiring.
- The `official` backend expects `config.yaml` to already point at EWMBench-style preprocessed directories and checkpoint paths. It does not convert an arbitrary flat video directory into a full EWMBench data layout.
- The public CLI path imports existing result rows. Generated videos alone do not produce EWMBench metrics.
- A leaderboard-equivalent claim needs the full dataset, preprocessing, DINOv2, YOLO-World, Qwen, and CLIP checkpoints, plus an upstream-compatible result table.
