...
- Raw-video scoring requires the upstream InternVL/CLIP/VQA judge assets and protocol, which are not run in tree (catalog blocker). WorldFoundry does not bundle those judge weights or credentials.
- Scorecard primary key: `phyeduvideo_average`.
