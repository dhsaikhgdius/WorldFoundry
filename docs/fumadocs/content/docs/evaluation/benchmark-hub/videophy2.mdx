...
Primary metric: `joint_score`. All metrics are higher-is-better and normalized to the unit interval.

| Metric | Meaning |
| --- | --- |
| `semantic_adherence` | Mean 1-5 rating for whether the video depicts the requested action, objects, and scene, normalized to the unit interval. |
| `physical_commonsense` | Mean 1-5 rating for whether the action and material interaction are physically plausible, normalized to the unit interval. |
| `joint_score` | Fraction of prompts where semantic adherence and physical commonsense are both at least 4/5. Primary scorecard key. |
| `rule_followed_rate` | Fraction of determined physical rules judged as followed rather than violated. |

The legacy ids `rule_classification_accuracy` and `videophy2_average` are deprecated: when present in an imported result file they are preserved under `components.deprecated_metrics` but are not reported as scorecard metrics.
...
- `scorecard.json`: WorldFoundry scorecard with `semantic_adherence`, `physical_commonsense`, `joint_score`, and `rule_followed_rate`.
...
- The AutoEval official runtime is integrated but has not been executed in this checkout; running it requires the `videophysics/videophy_2_auto` checkpoint and a CUDA GPU.
- Full validation requires generated videos for the official 200-prompt suite; leaderboard parity requires full upstream scorer execution and normalized official result files.
