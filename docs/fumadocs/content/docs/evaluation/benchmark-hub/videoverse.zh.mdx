...
scorecard 主键：`videoverse_average`。
...
- 默认 judge backend 是 hosted Gemini API（`gemini`，默认模型 `gemini-2.5-pro`），需要 API key；评审是付费的外部调用，不是本地计算。
- `WORLDFOUNDRY_VIDEOVERSE_JUDGE_BACKEND=mock` 仅用于接线测试；绝不要把 mock 评审当作基准结果报告。
