...
主指标是 `joint_score`。所有指标都是越高越好，并归一化到单位区间。

| Metric | 含义 |
| --- | --- |
| `semantic_adherence` | 1-5 分平均值（归一化到单位区间）：视频是否呈现了 prompt 要求的动作、物体和场景。 |
| `physical_commonsense` | 1-5 分平均值（归一化到单位区间）：动作和材料交互是否符合物理常识。 |
| `joint_score` | semantic adherence 和 physical commonsense 同时至少达到 4/5 的 prompt 比例。scorecard 主键。 |
| `rule_followed_rate` | 已判定物理规则中被判为「遵守」而非「违反」的比例。 |

旧指标 id `rule_classification_accuracy` 与 `videophy2_average` 已废弃：导入的结果文件中若出现，会保留在 `components.deprecated_metrics` 里，但不会作为 scorecard 指标报告。
...
- `scorecard.json`: WorldFoundry 统一 scorecard，包含 `semantic_adherence`、`physical_commonsense`、`joint_score` 和 `rule_followed_rate`。
...
- AutoEval 官方 runtime 已集成，但未在本 checkout 中执行过；运行它需要 `videophysics/videophy_2_auto` checkpoint 与 CUDA GPU。
- 完整验证需要官方 200 条 prompt 套件的生成视频；leaderboard 对齐需要完整的上游 scorer 执行与归一化的官方结果文件。
