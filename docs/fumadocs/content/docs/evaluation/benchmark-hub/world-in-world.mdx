...
- Catalog verification status is `result_importer_ready` with `leaderboard_valid: false` and `official_benchmark_verified: false`. Imported numbers are normalized copies of caller-provided results, not officially reproduced leaderboard values.
- Closed-loop rollout generation, environment execution, and official task scoring are outside the in-tree importer (catalog blocker); worker checkpoints (SAM2, FTWan2.1 LoRA, NWM) must be staged by the caller.
- Scorecard primary key: `world_in_world_average`.
