...
- judge 模式会调用 hosted GPT 评审，需要 `OPENAI_API_KEY` 或 `WORLDFOUNDRY_T2V_SAFETY_BENCH_GPT_API_KEY`；由于依赖用户凭据，发布验证期间未真正运行过评审。
- 完整验证需要所有官方 prompt 类别的全部生成视频；leaderboard 对齐要求使用与基准作者相同的评审模型与协议。
