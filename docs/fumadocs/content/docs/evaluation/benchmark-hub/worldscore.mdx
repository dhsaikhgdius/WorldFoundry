...
- Catalog flags are `leaderboard_valid: false` and `official_benchmark_verified: false` (release status `experimental`); bounded official GPU validation covers one dynamic sample only, and full-suite validation remains pending (catalog blockers).
- Leaderboard parity requires complete `evaluation.json` trees for the full 3,000-example suite plus the official metric checkpoint stack (DROID, GroundingDINO, SAM, RAFT/SEA-RAFT); imported results are normalized copies of caller-provided numbers.
- Scorecard primary key: `worldscore_average`.
