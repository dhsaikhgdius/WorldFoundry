...
T2V-CompBench maintains a live leaderboard at [Kaiyue/T2V-CompBench_Leaderboard](https://huggingface.co/spaces/Kaiyue/T2V-CompBench_Leaderboard). A submission zip contains at most eight CSVs:
...
The upstream paper evaluates 17 open-source and 6 commercial T2V systems across seven compositional categories. Each category expects 200 videos; the leaderboard backend drops any category with fewer than 200 scored rows. The final CSV for each category must end with a line starting `score:` or `Score:`.

WorldFoundry's `t2v_compbench_average` is the mean of the category scores available in the local scorecard. Treat the live leaderboard as authoritative for current rankings.
...
- Only the `generative_numeracy` component has been executed as a bounded one-video official run in this repository; that is not full-suite evidence.
- The four MLLM-based categories still require a compatible LLaVA runtime and checkpoint; `motion_binding` requires DOT, and the detection paths require their category-specific Depth-Anything/SAM assets.
- Full-suite verification requires all 1,400 generated videos (200 per category); full official benchmark and leaderboard parity have not been executed.
