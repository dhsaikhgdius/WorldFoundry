...
所有指标都是越高越好。每个 `<dimension>_score` 聚合是该组单项指标的均值，且仅在组内全部指标可用时才输出。主键 `wbench_average` 在五个维度分数齐全时取其均值。
...
- 官方 VLM 指标会调用 hosted VLM API，需要 `VLM_API_KEY`；本地视觉合理性打分还需要 PAVRM 模型目录。
- leaderboard 有效分数要求覆盖完整划分的完整官方报告；WorldFoundry 不会为官方 WBench 提交流程运行或托管你的模型。
