...
VideoScore is primarily a **reward model / automatic metric**, not a prompt-suite generation leaderboard. The key validation results reported in the paper:

| Setting | Spearman correlation with humans |
| --- | ---: |
| VideoFeedback-test | 77.1% |
| Previous best automatic metric | about 27% lower on average |

VideoScore-Bench aggregates roughly **7,000** videos drawn from [VideoFeedback](https://huggingface.co/datasets/TIGER-Lab/VideoFeedback), EvalCrafter, GenAI-Bench, and VBench splits. The official bench configs are `video_feedback`, `eval_crafter`, `genaibench`, and `vbench`.

WorldFoundry normalizes the five 1–4 aspect scores to 0–1 and reports `videoscore_average`. Use the full frame-tree evaluation for benchmark-level evidence; the bounded-sample path is for local validation only.

Primary scorecard key: `videoscore_average`.
...
- Bounded VideoScore scorer GPU validation over `video_feedback` exists as catalog evidence, but it is not a leaderboard submission.
- The full-suite SPCC/pairwise leaderboard protocol still requires complete upstream generated-frame trees and the VideoScore scorer-model weights.
