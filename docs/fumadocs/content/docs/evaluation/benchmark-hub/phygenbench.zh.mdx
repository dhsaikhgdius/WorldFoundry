...
- catalog 校验状态为 `bounded_component_ready`，`leaderboard_valid: false`、`official_benchmark_verified: false`。
- WorldFoundry 不运行 PhyGenEval 各阶段 judge（VQAScore/CLIP-FlanT5、GPT-4o 或 LLaVA-Interleave、InternVideo2）；仅做 overall 聚合不构成官方基准证据（catalog blocker）。
- 默认 fixture/mock 后端产出确定性接线数据，不得作为基准证据上报。
- scorecard 主键：`phygenbench_average`。
