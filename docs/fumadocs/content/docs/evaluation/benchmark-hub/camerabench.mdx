...
Primary scorecard key: `camerabench_average`.
...
- The public surface imports official result files or aggregates a prepared score directory; it does not generate VLM/SfM predictions. Producing official CameraBench score files still requires the upstream score-generation stack.
- Caption GPT judging is optional and only runs when explicitly enabled with an `OPENAI_API_KEY`; no hosted judging is executed by default.
- Full leaderboard validation still requires source-audited score generation over the complete test split and the upstream submission protocol.
