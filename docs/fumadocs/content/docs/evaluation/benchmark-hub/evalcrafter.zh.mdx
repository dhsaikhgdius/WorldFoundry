...
scorecard 主键：`evalcrafter_total`。
...
- 默认的树内运行只覆盖 CLIP 系列分数与人脸一致性；完整 17 指标套件仍依赖隔离环境中的 DOVER、OCR、动作识别、BLIP2、SDXL 与名人 ID 栈。
- 在全部 700 个官方视频上跑通 17 个原始指标与 5 个聚合之前，完整 EvalCrafter 与 leaderboard 用途仍处于受阻状态。
