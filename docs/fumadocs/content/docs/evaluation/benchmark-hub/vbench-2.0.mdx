...
- Bounded official GPU validation covers the Diversity dimension only; the full dimension stack requires the complete generated-video set, dataset assets, and materialized base-model checkpoints.
- Upstream Hugging Face license fields are null in API responses and need manual review before redistribution.
