...
- catalog 校验状态为 `pending`，`leaderboard_valid: false`、`official_benchmark_verified: false`（release status 为 `experimental`）。
- 完整校验需要官方 prompt suite 的生成视频以及官方 judge/checkpoint/API 资产（catalog blockers）；本集成不声称已完成完整 GPU 全套运行。
- naturalness 分量是 hosted Gemini judge：official runtime 要求提供 `--gemini-api-key`/`GEMINI_API_KEY` 或预计算的 `--naturalness-path` xlsx，两者都缺时会抛出 `ValueError`——不会静默跳过。
- scorecard 主键：`devil_dynamics_average`。
