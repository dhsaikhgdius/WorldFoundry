...
Primary scorecard key: `videoverse_average`.
...
- The default judge backend is the hosted Gemini API (`gemini`, default model `gemini-2.5-pro`) and requires an API key; judging is a paid external call, not a local computation.
- `WORLDFOUNDRY_VIDEOVERSE_JUDGE_BACKEND=mock` exists for wiring tests only; never report mock judgments as benchmark results.
