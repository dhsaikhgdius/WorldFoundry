...
- Judge mode calls the hosted GPT judge and requires `OPENAI_API_KEY` or `WORLDFOUNDRY_T2V_SAFETY_BENCH_GPT_API_KEY`; real judge execution was not run during release validation because it requires user credentials.
- Full validation requires every generated video in all official prompt classes; leaderboard parity requires the same judge model and protocol used by the benchmark authors.
