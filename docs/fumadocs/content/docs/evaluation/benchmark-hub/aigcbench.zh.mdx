...
- 公开入口本质是结果导入/归一化器：`--run-official` 只有在本地已配置好 WorldFoundry scorer 栈时才会产出指标行。树内 runner 并未重新实现完整的 CLIP/DOVER/光流管线。
- 完整验证需要官方 prompt 套件的生成视频，以及保真度与参考视频指标族所需的参考图像和参考视频。
