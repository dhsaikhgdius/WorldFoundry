...
- 当 `WORLDFOUNDRY_GENAI_BENCH_SCORER_BACKEND`（或 `WORLDFOUNDRY_T2V_METRICS_BACKEND`）未设置时，scorer backend 默认是 `mock`。mock 偏好预测是确定性的占位值；真实评分必须把 backend 设为 `vqascore`，绝不要报告 mock 数值。
- 完整 leaderboard 验证需要官方人工偏好标签或评审输出，以及官方成对 prompt/任务套件的生成产物。
