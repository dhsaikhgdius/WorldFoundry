...
scorecard 主键：`camerabench_average`。
...
- 公开入口只能导入官方结果文件或聚合已准备好的 score 目录，不会生成 VLM/SfM 预测。产出官方 CameraBench score 文件仍需上游的分数生成栈。
- caption GPT 评审是可选项，只有显式启用并提供 `OPENAI_API_KEY` 时才运行；默认不执行任何 hosted 评审。
- 完整 leaderboard 验证仍需在完整测试划分上做经源码审计的分数生成，并遵循上游提交协议。
