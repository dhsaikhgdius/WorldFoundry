...
- Full official-run mode calls a hosted VLM judge and requires a configured judge provider plus API key; import mode only normalizes existing judge outputs.
- Full validation requires generated videos for the official experiment suite; leaderboard parity requires official judge assets and normalized upstream result files.
