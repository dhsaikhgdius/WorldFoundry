...
- VideoCon-Physics 官方 runtime 已集成，但未在本 checkout 中执行过；运行它需要 `videophysics/videocon_physics` checkpoint 与 CUDA GPU。
- 完整验证需要官方 prompt 套件的生成视频；leaderboard 对齐需要完整的上游 scorer 执行与归一化的官方结果文件。
