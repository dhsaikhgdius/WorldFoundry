...
- The public surface is a result importer/normalizer: `--run-official` only produces metric rows when a locally configured WorldFoundry scorer stack exists. The in-tree runner does not reimplement the full CLIP/DOVER/optical-flow pipeline.
- Full validation requires generated videos for the official prompt suite plus reference images and reference videos for the fidelity and reference-video metric families.
