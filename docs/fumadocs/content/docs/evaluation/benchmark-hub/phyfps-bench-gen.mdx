...
- A mock prediction backend exists (`WORLDFOUNDRY_PHYFPS_PREDICT_BACKEND=mock`); scorecards produced with it are flagged as mock runtime and are not official evidence. The default backend is `official`.
- Meta FPS alignment metrics require caller-supplied Meta FPS values; full leaderboard parity requires the complete official prompt suite, generated artifacts, and the Visual Chronometer checkpoint.
