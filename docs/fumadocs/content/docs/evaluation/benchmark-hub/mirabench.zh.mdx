...
- `WORLDFOUNDRY_MIRABENCH_SCORER_BACKEND=mock` 只为接线运行写入确定性的占位指标值；绝不要把 mock 数值当作基准结果报告。
- 完整验证需要官方 150 条 prompt 套件的生成视频与已就位的 scorer checkpoint；leaderboard 对齐还需要官方评审/checkpoint 资产与归一化的上游结果文件。
