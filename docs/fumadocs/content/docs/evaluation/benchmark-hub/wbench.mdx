...
| Dimension | Aggregate metric id | Per-metric ids |
| --- | --- | --- |
| Quality | `quality_score` | `aesthetic_quality`, `imaging_quality`, `temporal_flickering`, `dynamic_degree`, `motion_smoothness`, `hpsv3_quality` |
| Setting | `setting_score` | `scene_adherence`, `subject_adherence` |
| Interaction | `interaction_score` | `navigation_trajectory`, `event_edit_adherence`, `subject_action_adherence`, `perspective_switch_adherence` |
| Consistency | `consistency_score` | `background_consistency`, `segment_continuity`, `perspective_consistency`, `subject_consistency`, `geometric_consistency`, `photometric_consistency`, `spatial_consistency`, `gated_spatial_consistency` |
| Physical | `physical_score` | `visual_plausibility`, `causal_fidelity` |

All metrics are higher-is-better. Each `<dimension>_score` aggregate is the mean of its per-metric ids and is only emitted when every metric in the group is available. The primary key `wbench_average` is the mean of the five dimension scores when all five are present.
...
- The official VLM metrics call a hosted VLM API and require `VLM_API_KEY`; local visual-plausibility scoring additionally needs the PAVRM model directory.
- Leaderboard-valid scores require a complete official report over the full split; WorldFoundry does not run or host your model for the official WBench submission process.
- This page documents runnable commands; it does not claim those GPU runs were executed in this environment.
