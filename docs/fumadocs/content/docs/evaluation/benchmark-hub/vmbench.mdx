...
- All metrics run from local checkpoints (no hosted judge), but full recomputation requires the CUDA metric stack: GroundingDINO, SAM/SAM2, CoTracker, Q-Align, and the VMBench CAS checkpoint.
- The `AMAP-ML/VMBench` Hugging Face dataset endpoint was not public at integration time; the prompt suite is bundled in tree under `worldfoundry/data/benchmarks/assets/vmbench`. Generated videos must follow the official `0001.mp4` … `1050.mp4` naming.
