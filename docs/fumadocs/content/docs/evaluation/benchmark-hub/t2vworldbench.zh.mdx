...
`WORLDFOUNDRY_T2VWORLDBENCH_PROMPT_FILE` 是 runner 读取的唯一 prompt 相关环境变量；官方 runtime 的树内位置为 `worldfoundry/evaluation/tasks/execution/runners/t2vworldbench/runtime/t2vworldbench`，随仓库捆绑的 prompt CSV 位于 `worldfoundry/data/benchmarks/assets/t2vworldbench/prompt/prompt.csv`。
...
- 随仓库捆绑的 `meta_data.json` 只是一个很小的树内示例；完整验证需要生成视频，以及手工准备的官方 Drive 元数据与样例资产。
- leaderboard 对齐需要上游 VLM runtime（LLaVA 兼容 checkpoint）与归一化的官方结果文件。
