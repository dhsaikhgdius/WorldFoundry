...
- The default in-tree run covers only the CLIP-based scores and face consistency; the full 17-metric suite still requires the isolated DOVER, OCR, action-recognition, BLIP2, SDXL, and celebrity-ID stack.
- Full EvalCrafter and leaderboard use remain blocked until all 17 raw metrics and five aggregates run over all 700 official videos.
