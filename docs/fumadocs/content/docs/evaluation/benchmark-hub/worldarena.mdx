...
- Catalog verification status is `in_tree_video_quality_runtime` with `leaderboard_valid: false` and `official_benchmark_verified: false` (release status `experimental`).
- Full validation still requires generated videos or rollouts for the official prompt/task suite; leaderboard parity requires official judge/checkpoint/API assets plus embodied task utility evidence (catalog blockers).
- Result import via `--official-results-path` normalizes caller-provided numbers; it is not fresh benchmark execution.
- Scorecard primary key: `ewm_score`.
