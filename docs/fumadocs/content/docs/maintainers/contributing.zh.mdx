...
文档 codegen（`npm run predev` / `prebuild` → `api:generate`、`models:generate` 等）经
`docs/fumadocs/scripts/run-python.sh` 调用解释器。主机 PATH 没有 `python3` 时，可用
`WF_DOCS_PYTHON`（其次 `PYTHON`，再 `python3` / `python`）覆盖。
