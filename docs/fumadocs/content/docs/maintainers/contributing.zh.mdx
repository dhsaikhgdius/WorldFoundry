...
- **新模型或 runtime profile** — `guides/supported-models`、`reference/environments`，行为变化时更新模型指南。刷新首页 recipes：`cd docs/fumadocs && npm run models:generate`，再用 `npm run models:check`。
