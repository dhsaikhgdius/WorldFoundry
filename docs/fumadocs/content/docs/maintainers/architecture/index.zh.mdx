...
<Cards>
  <Card title="代码地图" description="全包“目录 → 职责”速查表，含每个包的首读文件。" href="/zh/docs/maintainers/architecture/file-index" />
  <Card title="使用界面与编排" description="CLI、TUI、MCP 如何把用户意图编译为三个规划边界。" href="/zh/docs/maintainers/architecture/surfaces-orchestration" />
  <Card title="工作流" description="从 CLI 输入追踪到产物、指标、scorecard 和报告。" href="/zh/docs/maintainers/architecture/workflow" />
  ...
  <Card title="Runtime 组装" description="pipeline、operator、synthesis、representation、base_models 的基类契约与目录职责。" href="/zh/docs/maintainers/architecture/runtime-assembly" />
  <Card title="原生 Diffusion" description="理解框架自有的采样内核、组件边界、依赖规则和研究扩展点。" href="/zh/docs/maintainers/architecture/native-diffusion" />
  <Card title="核心基础设施" description="worldfoundry/core 共享层——加载、attention、分布式、VRAM、kernel、memory，以及“可复用原语只在此处”的规则。" href="/zh/docs/maintainers/architecture/core-infrastructure" />
...
下方七条职责是权威的归属模型——每条写明该层拥有什么、不能伸手到哪里。[设计](/zh/docs/overview/design) 的四层图只是粗粒度高层概览，可映射到这七条上；当改动涉及归属时，以本列表为准，而非那张图。
...
- **原生 Diffusion** 位于 `worldfoundry/base_models/diffusion_model`，负责本地 diffusion 组件契约和采样循环，不能依赖任何上游 pipeline 框架，也不能反向依赖 WorldFoundry 的编排与评测层。
...
- **唯一原生 diffusion 内核** — 上游仓库只作为实现参考，不作为 runtime backend；可复用的推理算法在 WorldFoundry 内只有一份 canonical implementation。
