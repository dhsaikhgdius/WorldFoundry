...
Every evaluation run—whether it loads a model or scores existing files—passes through one facade (`run_evaluate`), which validates the request, picks a delegate runner (`ContractRunner`, `ExistingResultsRunner`, or `ModelBenchmarkRunner`), and hands off to metrics and reporting. The facade stays thin: all file writing, sample ledgers, and manifest updates happen inside the delegate runners. For the stage-by-stage run flow, the mode-fork detail, and which file each stage writes, see [Workflow](/docs/maintainers/architecture/workflow).

![Evaluation run flow: request to scorecard](/diagrams/flow-evaluation-core.svg)

User-facing CLI, TUI, and Workspace flows compile into the request types this facade consumes through `execution/orchestration/service.py`. The five intent dataclasses, the three planning boundaries (`PreparedEvaluation`, `RunPlan`, `ModelBenchmarkRunRequest`), and the compile entrypoints are mapped in [Surfaces and orchestration](/docs/maintainers/architecture/surfaces-orchestration). Frontends must not duplicate generation, artifact materialization, benchmark dispatch, or metric execution; new UX should add an intent-to-request mapping, not another runner. Do not wrap these in a fourth recipe executor.
...
Disk YAML becomes a runnable model through a single front door: `resolve_model_zoo_runner()` in `resolver.py` (`:365`). CLI `--model-id` flags, Python callers, and Studio previews should all converge here before any checkpoint path is interpreted.
...
Validation scripts and docs generators import `reporting/validation.py` and expect stable scorecard fields. Field renames here break release gates. The concrete entry points: `build_scorecard` at `reporting/scorecard.py:131`, `write_scorecard` at `:248`, `build_run_summary` at `reporting/run_report.py:174`, and `_validate_scorecard` at `reporting/validation.py:207`.
