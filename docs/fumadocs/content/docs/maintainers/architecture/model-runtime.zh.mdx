...
Pipeline class
  ...
  -> Synthesis: call a native model runtime or hosted API
     -> Native diffusion engine: condition/initialize/denoise/decode（仅本地 diffusion）
...
![模型 runtime 组装链](/diagrams/flow-model-runtime.zh.svg)
...
`resolve_pipeline_route()`（`evaluation/models/pipelines/bindings.py:307`）与 `resolve_model_zoo_runner()`（`evaluation/models/runners/resolver.py:365`）把这条链路解析成可运行 runner。其背后的 catalog 解析、spec 合成与 runner 解析机制见 [评测核心 — 模型解析](/zh/docs/maintainers/architecture/evaluation-core)。Planned 或 profile-only 条目可以保留 `runtime_profile`，但不应暴露 runner target；薄 planning wrapper 也不应把 `pipeline_target` 误报成已验证支持。
