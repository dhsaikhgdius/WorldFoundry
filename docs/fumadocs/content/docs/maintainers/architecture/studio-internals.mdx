...
browser key edges
  ...
  -> ResidentWorldRuntime (one worker thread)
  -> realtime_frames_from_result()
  -> VideoPostprocessStream (one session-scoped chain)
  -> resolution/JPEG transport work
  -> latest-interactive or ordered-quality queue
  -> WebRTC / WebSocket
```
