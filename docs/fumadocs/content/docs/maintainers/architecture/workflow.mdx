...
Every user-facing command eventually reaches `run_evaluate()` in `evaluation/tasks/execution/orchestration/evaluate.py` (`run_evaluate` at `:1057`). The console entry is `worldfoundry/cli/main.py:1580` `main()`, which dispatches subcommands into one of four shapes:
...
Important distinction: model execution and benchmark scoring are separated. A model runner only emits `GenerationResult` objects. Scoring happens after results are normalized. `ContractRunner` (`orchestration/contract.py:92`) and `ExistingResultsRunner` (`orchestration/existing_results.py:89`) are the two delegate runners this fork chooses between; model resolution uses `resolve_model_zoo_runner()` at `evaluation/models/runners/resolver.py:365`.

## Contract runner detail

`run_contract()` is the strictest in-process path and the reference implementation for artifact layout (`run_contract` at `evaluation/tasks/execution/orchestration/contract.py:848`, `ContractRunner` at `:92`). Each stage writes or updates a file that downstream validation, docs exports, and leaderboard tooling expect to find. When debugging a partial run, walk the stages in order: a missing `metrics/per_sample.jsonl` usually means generation never finished alignment, not that reporting failed.
