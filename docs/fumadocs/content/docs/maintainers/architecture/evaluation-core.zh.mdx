...
无论是否加载模型，每次 evaluation run 都经过同一个 facade（`run_evaluate`），它校验请求、选择 delegate runner（`ContractRunner`、`ExistingResultsRunner` 或 `ModelBenchmarkRunner`），再交给 metrics 与 reporting。facade 保持薄：所有文件写入、sample ledger 与 manifest 更新都在 delegate runner 内完成。阶段级 run 流程、mode 分叉细节以及每个阶段写哪个文件，见 [工作流](/zh/docs/maintainers/architecture/workflow)。

![评测 run 流程：request 到 scorecard](/diagrams/flow-evaluation-core.zh.svg)

面向用户的 CLI、TUI 与 Workspace 通过 `execution/orchestration/service.py` 编译成本 facade 消费的 request 类型。五种 intent dataclass、三个规划边界（`PreparedEvaluation`、`RunPlan`、`ModelBenchmarkRunRequest`）以及编译入口见 [使用界面与编排](/zh/docs/maintainers/architecture/surfaces-orchestration)。前端不能重复实现 generation、artifact materialization、benchmark dispatch 或 metric execution；新增 UX 应增加 intent→request 映射，而非新增 runner。不要再包第四种 recipe executor。
...
磁盘 YAML 经唯一 front door `resolver.py` 中的 `resolve_model_zoo_runner()` 变为可运行 model（`:365`）。CLI 的 `--model-id`、Python caller 与 Studio 预览都应在解释 checkpoint 路径前汇聚于此。
...
Validation 脚本与 docs generator 会 import `reporting/validation.py`，并期望 scorecard field 稳定。在此重命名 field 会破坏 release gate。具体入口：`build_scorecard` 在 `reporting/scorecard.py:131`、`write_scorecard` 在 `:248`、`build_run_summary` 在 `reporting/run_report.py:174`、`_validate_scorecard` 在 `reporting/validation.py:207`。
