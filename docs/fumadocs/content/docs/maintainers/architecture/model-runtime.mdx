...
Pipeline class
  ...
  -> Synthesis: call a native model runtime or hosted API
     -> Native diffusion engine: condition/initialize/denoise/decode (local diffusion only)
...
![Model runtime assembly chain](/diagrams/flow-model-runtime.svg)
...
`resolve_pipeline_route()` (`evaluation/models/pipelines/bindings.py:307`) and `resolve_model_zoo_runner()` (`evaluation/models/runners/resolver.py:365`) turn that chain into a runnable runner. The catalog-parsing, spec-merging, and runner-resolution mechanics behind them are documented in [Evaluation Core — Model Resolution](/docs/maintainers/architecture/evaluation-core). Planned or profile-only entries may keep `runtime_profile` but should not expose a runner target; thin planning wrappers must not advertise `pipeline_target` as verified support.
