...
每条用户可见命令最终都会到达 `evaluation/tasks/execution/orchestration/evaluate.py` 中的 `run_evaluate()`（`run_evaluate` 在 `:1057`）。控制台入口是 `worldfoundry/cli/main.py:1580` `main()`，子命令会分发到以下四种形态之一：
...
关键点：模型执行和 benchmark scoring 是分开的。Model runner 只输出 `GenerationResult`；结果归一化后才评分。`ContractRunner`（`orchestration/contract.py:92`）与 `ExistingResultsRunner`（`orchestration/existing_results.py:89`）是此分叉选择的两个 delegate runner；模型解析用 `evaluation/models/runners/resolver.py:365` 的 `resolve_model_zoo_runner()`。

## Contract Runner 细节

`run_contract()` 是最严格的 in-process path，也是 artifact 布局的参考实现（`run_contract` 在 `evaluation/tasks/execution/orchestration/contract.py:848`，`ContractRunner` 在 `:92`）。每个阶段都会写出或更新下游 validation、docs export 与 leaderboard 工具期望找到的文件。调试不完整 run 时按阶段顺序排查：缺少 `metrics/per_sample.jsonl` 通常是 generation 未完成对齐，而非 reporting 失败。
