...
- **New model or runtime profile** — `guides/supported-models`, `reference/environments`, and the model guide if behavior changes. Refresh homepage recipes with `cd docs/fumadocs && npm run models:generate`, then `npm run models:check`.
