...
Prefer the smallest Makefile gate layer that covers the change:

- `make check-fast` — ruff + shell + `docs-check` (no catalog / compileall)
- `make check` — `lint` + `docs-check` (full static and catalog gates)
- `make check-ci` — `check` + `cli-check` (CPU evaluate hello-world scorecard)
