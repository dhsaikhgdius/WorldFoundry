...
Docs codegen (`npm run predev` / `prebuild` → `api:generate`, `models:generate`, …) goes through
`docs/fumadocs/scripts/run-python.sh`. Override the interpreter with `WF_DOCS_PYTHON` (then
`PYTHON`, then `python3` / `python`) when the host PATH has no `python3`.
