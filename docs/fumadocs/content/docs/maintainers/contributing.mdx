...
- **Public Python API docstrings** — regenerate with `npm run api:generate` under `docs/fumadocs`, then keep the tree clean with `npm run api:check`.
...
make lint
make docs-check
cd docs/fumadocs && npm run api:check && npm run types:check && npm run build
```
