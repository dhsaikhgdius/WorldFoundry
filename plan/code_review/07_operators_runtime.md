...
```
  ...
  实测（本仓库布局 `<...>/WorldFoundry/worldfoundry/runtime/probes.py`）：`parents[2]` = 仓库根，`parents[4]` = `<VISUAL_GENERATION_ROOT>`（仓库外两级）。`performance.py:517` 用的是正确的 `parents[2]`。`evaluation/utils.py:153` 则有第四种：`worldfoundry_repository_root()`。
- **问题**：
