...
1. **TR-12/13/14（ray 生命周期，P1+P2+P2）**：位置全在 `worldfoundry/training/distributed/ray_runtime.py`（setup 失败泄漏 placement group `:394-415`、`ready()` 无超时 `:413`、shutdown 吞错+无重启策略 `:493-497`）——`distributed/` 明确排除在本次授权外（另一 agent 负责），未动。**后续更新：TR-12/13 已由 infra 任务 6（分支 `cursor/infra-code-review-fixes-2f62`）修复**：setup 失败回滚 placement group + 自启动会话 shutdown、新增 `placement_timeout_seconds` 配置与 `placement_group_table()` 诊断；验证 `tests/training/test_training_fix_ray_device_pool_lifecycle.py` 8 条通过（详见 `second_round_fixes.md`）。TR-14（重启策略）仍 deferred。本 agent 范围内的 ray 失败路径已独立核查完毕：`agentic/remote.py:428-447`（`setup_ray_agentic_rollout` 失败 → `runtime.shutdown()` 后 `raise`）、`causal_lm/qwen3/materializer.py` 三条构建路径（`:593-628` actor-hosted setup 失败 shutdown、`:695-708` external 失败关闭已建 closeables、`:712-738` materialize 失败逆序 close/shutdown 后 raise）均有清理且不吞错；TR-15 的错误信息丢弃已修。
