.PHONY: help install-core install-dev docs-check lint ruff-check ruff-format-check syntax-check format-check shell-check data-check runtime-registry-check compile-eval cli-check precommit precommit-install preflight test-eval-core test-training
...
# Optional until RUFF_SOURCES are format-clean (pre-existing format drift on main);
# not wired into lint yet (plan C-05).
ruff-format-check:
	$(PYTHON) -m ruff format --check $(RUFF_SOURCES)

# Byte-compile the canonical diffusion package sources only (plan C-05).
# Formerly misnamed format-check; worldfoundry/evaluation and scripts are
# already covered by compile-eval, so they are not duplicated here.
syntax-check:
