.PHONY: help install-core install-dev docs-check lint ruff-check format-check shell-check data-check parents-ckpt-check runtime-registry-check compile-eval cli-check precommit precommit-install preflight test-eval-core test-training
...
lint: ruff-check format-check shell-check data-check parents-ckpt-check runtime-registry-check
...
parents-ckpt-check:
	PYTHONPATH=$(PYTHONPATH) $(PYTHON) scripts/ci/check_no_parents_ckpt.py
