...
class TextEncoder(nn.Module):
    ...
    def text2tokens(self, text, data_type="image"):
        ...
        if tokenize_input_type == "str":
            ...
            return self._expand_i2v_image_tokens(encoding, data_type)
    ...
    def _expand_i2v_image_tokens(self, batch_encoding, data_type="image"):
        if not self.i2v_mode:
            return batch_encoding

        template = self.prompt_template_video if data_type == "video" else self.prompt_template
        image_emb_len = int((template or {}).get("image_emb_len", 576))
        if image_emb_len <= 1:
            return batch_encoding
        ...
        for row_idx, row in enumerate(input_ids):
            image_positions = (row == image_token_id).nonzero(as_tuple=False).flatten()
            if image_positions.numel() != 1:
                expanded_ids.append(row)
                if attention_mask is not None:
                    expanded_masks.append(attention_mask[row_idx])
                continue
            pos = int(image_positions[0].item())
            image_tokens = row.new_full((image_emb_len,), image_token_id)
            expanded_ids.append(torch.cat([row[:pos], image_tokens, row[pos + 1 :]], dim=0))
            if attention_mask is not None:
                mask_row = attention_mask[row_idx]
                image_mask = mask_row.new_ones((image_emb_len,))
                expanded_masks.append(torch.cat([mask_row[:pos], image_mask, mask_row[pos + 1 :]], dim=0))

        max_len = max(row.numel() for row in expanded_ids)
        pad_id = self.tokenizer.pad_token_id
        if pad_id is None:
            pad_id = self.tokenizer.eos_token_id or 0
        padded_ids = []
        padded_masks = []
        for idx, row in enumerate(expanded_ids):
            pad = max_len - row.numel()
            if pad > 0:
                row = torch.cat([row, row.new_full((pad,), pad_id)], dim=0)
            padded_ids.append(row)
            if attention_mask is not None:
                mask_row = expanded_masks[idx]
                if pad > 0:
                    mask_row = torch.cat([mask_row, mask_row.new_zeros((pad,))], dim=0)
                padded_masks.append(mask_row)
        batch_encoding["input_ids"] = torch.stack(padded_ids, dim=0)
        if attention_mask is not None:
            batch_encoding["attention_mask"] = torch.stack(padded_masks, dim=0)
        return batch_encoding
        ...
        if not self.i2v_mode:
            ...
            if self.use_template:
                if data_type == "video":
                    ...
                    attention_mask_assistant_crop_start = assistant_crop_start
                    attention_mask_assistant_crop_end = assistant_crop_end
                else:
                    raise ValueError(f"Unsupported data type: {data_type}")
                    ...
                    text_attention_mask.append(
                        torch.cat(
                            [
                                attention_mask[
                                    i,
                                    text_crop_start: attention_mask_assistant_crop_start[i].item(),
                                ],
                                attention_mask[
