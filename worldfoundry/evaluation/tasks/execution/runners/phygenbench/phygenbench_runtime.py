...
def _run_upstream_overall(*, repo_root: Path, model_name: str) -> Path:
    ...
    stdout_path = Path(repo_root) / "result" / f"{model_name}.overall.stdout.log"
    stderr_path = Path(repo_root) / "result" / f"{model_name}.overall.stderr.log"
    ...
    if completed.returncode != 0:
        detail = (stderr_path.read_text(encoding="utf-8").strip() or stdout_path.read_text(encoding="utf-8").strip())
        raise RuntimeError(
