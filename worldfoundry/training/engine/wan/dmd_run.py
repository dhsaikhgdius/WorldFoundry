"""Wan DMD run lifecycle, metrics, checkpoint boundaries, and export."""
...
from .roles import WanDMDRoleBundle

WAN_DMD_RUN_SCHEMA = "worldfoundry-wan-dmd-run"


class WanDMDTrainingRun:
    ...
    def _student_export_metadata(self) -> dict[str, object]:
        return {
            "run_id": self.recipe.run.id,
            "recipe_digest": self.recipe.digest,
    ...
    def _export_student_artifact(
        ...
        *,
        ...
        step = self.session.engine.global_step
        metadata = self._student_export_metadata()
            ...
            if export_format == "peft":
                ...
                if destination.exists():
                    ...
                    if dict(artifact.metadata) != metadata:
                        raise ValueError("existing DMD student artifact metadata differs")
                else:
    ...
    def export_student(
        ...
        """Export the configured, digest-audited student artifact."""
    ...
    def export_student_peft(
        ...
        if not isinstance(artifact, PeftAdapterArtifact):
            raise RuntimeError("configured DMD export is not PEFT")
        return artifact
