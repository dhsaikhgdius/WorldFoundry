...
- **Pinned dependency versions (do not "upgrade" blindly)**:
  ...
  - `gradio>=5.50,<6` + `starlette<1.0` — the Studio `workspace_app` uses `app.add_event_handler`, which was removed in Starlette 1.0 / Gradio 6. The `[ui]` / `[all]` extras pin these bounds so a bare `pip install -e ".[ui]"` cannot pull Gradio 6. Keep the pin until the app migrates to lifespan.
