...
- **Pinned dependency versions (do not "upgrade" blindly)**:
  ...
  - `gradio>=5.50,<6` + `starlette<1.0` — the Studio `workspace_app` uses `app.add_event_handler`, which was removed in Starlette 1.0. Keep these pins in the `[ui]` / `[all]` extras; a bare unpinned install still pulls Gradio 6 / Starlette ≥1.0 and fails on startup with `'FastAPI' object has no attribute 'add_event_handler'`.
...
- **`make lint` is green** for the public-surface ruff sources (`worldfoundry/cli`, `worldfoundry/mcp`, evaluation API/runtime surfaces, and listed scripts). Prefer `ruff==0.12.7` when reproducing locally.
