...
| Lint / quality gates | `make lint`, `make compile-eval`, `make docs-check` | `make lint` should be green on this branch; pin `ruff==0.12.7`. |
...
- **Pinned dependency versions (do not "upgrade" blindly)**:
  - `ruff==0.12.7` — matches `.pre-commit-config.yaml` and `make install-dev`; newer ruff changes import-sort output and reports spurious lint diffs.
  - `gradio>=5,<6` + `starlette<1.0` — already pinned in the `[ui]` / `[all]` extras. The Studio `workspace_app` still uses `app.add_event_handler`, which was removed in Starlette 1.0; do not loosen these pins until that migrates to lifespan.
